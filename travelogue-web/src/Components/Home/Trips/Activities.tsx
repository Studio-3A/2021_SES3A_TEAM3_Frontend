...
const TripDay = (props: IState) => {
    return (
        <div className="tripday-block">
            {props.trip.map(a => <Activity activity={a}/>)}
        </div>
    );
}
...
const ActivityProperty = (props: BitProps) => {
    ...
    return (
        <>
            {value && 
                <div className={`activity-label ${className}`}>{value}</div>
            }
        </>
    );
};
...
const Activity = (props: ActProps) => {
    ...
    const actualTime = time != null && duration != null ? (moment(time).format(timeFormat) + ' - ' + moment(time).add({minute: duration}).format(timeFormat)) : time;
    ...
    return (
        <div className="activity-block">
            <div className="activity-card card card-shadow">
                ...
                <ActivityProperty label={"Price"} value={formattedPrice} className={"activity-price"}  />
                <ActivityProperty label={"Location"} value={location} className={"activity-location"} />
                </div>
            </div>
