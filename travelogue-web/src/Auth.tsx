// TODO:
//   -  Change out any for JSX.Element in Auth function signature
//   -  Work out the best way to provide the user/auth data to children components
//   -  Parameterise the server request so that the target server can be easily changed
...
// TODO:
function Auth({ children }: { children: Array<any> | any }) {
    ...
    async function checkToken(): Promise<boolean> {
        return new Promise(async (resolve, reject) => {
            try {
                // Contact the backend serve (Express) to check if your session is valid
                const result = await Axios.post("http://localhost:4000/auth/checkToken", { token: await getToken() });
